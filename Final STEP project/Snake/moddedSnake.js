...
let cellW = Math.floor(canvas.width / cellCountX * 0.9);
//пропорция клеток по Y
let cellH = Math.floor(canvas.height / cellCountY * 0.9);

const eatSound = new Audio('appleCrunch.m4a');
const lose = new Audio('lose.mp3');
const win = new Audio('fanfare.m4a');

const color = 'red blue navy yellow purple orange lime gold'.split(' ');
...
let score = 0;
let moddedHiScore = 'moddedHiScore';
let snakeHeadX = random(cellCountX);
let snakeHeadY = random(cellCountY);

let fruitX = random(cellCountX);
let fruitY = random(cellCountY);
...
function updateGame(){
    ...
    if (defeatCondition()){
        drawSnake();
        ctx.fillText('ЛУЧШИЙ РЕЗУЛЬТАТ: ' + localStorage.getItem(moddedHiScore),canvas.width * 0.55, canvas.height * 0.04);
        ctx.fillStyle = 'red';
...
function drawSnake(){
    ...
    //BUG: snake may last in astral while out of canvas or border-bottom glitch 
    ctx.fillRect(snakeHeadX * cellCountX, snakeHeadY * cellCountY, cellW,cellH);
    if(snakeHeadX < 0){
        snakeHeadX = Math.floor(cellCountX);
    }
    else if(snakeHeadY < 0){
        snakeHeadY = Math.floor(cellCountY);
    }    
    else if(snakeHeadX >= cellCountX){
        snakeHeadX = -1;
    }    
    else if(snakeHeadY >= cellCountY){
        snakeHeadY = -1;
    }
}
...
function drawFruit(){
    for (let i = 0; i < snakeParts.length; i++){
        ...
        if(fruitX == part.x && fruitY == part.y){
            fruitX = random(cellCountX);
            fruitY = random(cellCountY);
        }
    }
...
function whenFruitEaten(){
    if(snakeHeadX == fruitX && snakeHeadY == fruitY){
        ...
        tail++;
        score++;
        if(score > localStorage.getItem(moddedHiScore)){
            localStorage.setItem(moddedHiScore, score);
        }
        eatSound.play();
